...
import { AlertTriangle, Clock, AlertCircle, X, ChevronLeft, ChevronRight } from "lucide-react"
...
const TestPanel = () => {
  ...
  const getSubmissionSummary = () => {
    ...
    const unattemptedQuestions = shuffledQuestions.map((q, index) => ({ ...q, index })).filter((q) => !answers[q._id])
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="fixed left-4 top-4 z-50 rounded-lg overflow-hidden shadow-lg border-2 border-gray-700">
        {cameraError ? (
          <div className="bg-red-500/20 text-red-500 p-4 text-sm">
            <AlertCircle className="inline-block mr-2" size={16} />
            {cameraError}
          </div>
        ) : (
          <video ref={videoRef} autoPlay playsInline muted className="w-[180px] h-[135px] bg-black" />
        )}
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        {warningCount > 0 && (
          <motion.div
            ...
            className="bg-red-500/20 text-red-500 p-4 rounded-lg mb-6 mt-16 sm:mt-0"
          >
            Warning {warningCount}/{maxWarnings}: Further violations will result in automatic submission
          </motion.div>
        )}

        <div className="flex justify-between items-center mb-8 mt-16 sm:mt-0">
            ...
            >
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button onClick={() => setShowSubmitModal(false)} className="p-1 hover:bg-gray-700 rounded-lg">
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-end gap-4 mt-6">
                    <button onClick={() => setShowSubmitModal(false)} className="px-4 py-2 bg-gray-700 rounded-lg">
                      Review Test
                    </button>
                    <button onClick={() => handleSubmit()} className="px-4 py-2 bg-green-500 rounded-lg">
                      Submit Test
                    </button>
                  </div>
                </div>
